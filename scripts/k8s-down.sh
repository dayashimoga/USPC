...
$KUBECTL_BIN delete -k "$REPO_ROOT/deploy/k3s" -n "$NAMESPACE" --ignore-not-found=true || true

if [ "$PURGE_DATA" = "--purge-data" ] || [ "$PURGE_DATA" = "true" ]; then
    echo "==> Step 3: Purging PVCs and namespace..."
    $KUBECTL_BIN delete pvc --all -n "$NAMESPACE" --ignore-not-found=true || true
    $KUBECTL_BIN delete namespace "$NAMESPACE" --ignore-not-found=true || true
fi
